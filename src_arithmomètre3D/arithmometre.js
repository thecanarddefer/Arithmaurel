...
var objects = [],
    ...
    axeEngr = [],
    cursRetenu = [],
    axeManivelle, axePinc, parquet, dae;
var sysRetourRetenue = [],
    camAntiDepas = [],
    boiteChariot, barreAddSub, boiteAvant, boiteArriere, axeActionCompteur = [],
    boiteDessus, curseurActionCompteur, plaque = [];
var compteur = [],
    engrAccu = [],
    accumulateur = [],
    pignonRAZAccu, pignonRAZCompteur, peigneAccu, peigneCompteur, scaleDefault;
var TargetPos = [],
    ValCurProg;
var toucheBloque = false;
var reverseurSurAdd = true;
var positionIntialCurseurs = [],
    positionInitialMesh = [];
var positionIntialcurseurRetenue, positionInitialAntiDepas;
var remetreBoite = false;
var remetreChariot = false;
var valNumAccu = 0;
...
function init() {
    ...
    groupChariot.add(peigneAccu);
    scene.remove(peigneAccu);
    groupChariot.add(peigneCompteur);
    scene.remove(peigneCompteur);
    groupChariot.add(pignonRAZAccu);
    scene.remove(pignonRAZAccu);
    groupChariot.add(pignonRAZCompteur);
    scene.remove(pignonRAZCompteur);
    for (var i = 0; i < accumulateur.length; i++) {
        groupChariot.add(accumulateur[i]);
        scene.remove(accumulateur[i]);
    }
    for (var i = 0; i < compteur.length; i++) {
        groupChariot.add(compteur[i]);
        scene.remove(compteur[i]);
    }
    ...
    for (var k = 1; k < curseurRetenue.length; k++) { curseurRetenue[k].position.x += 0.03 }
    ...
    var patientez = document.getElementById('Patienter');
    patientez.parentNode.removeChild(patientez);
    VuExemple(true);
...
function onDocumentMouseDown(event) {
    ...
    if (intersects.length > 0) {
        ...
        if (SELECTED && (SELECTED.name == "renverseur" || SELECTED.name == "manivelle" ||
                SELECTED.name == "RAZtotalisateur" || SELECTED.name == "RAZeroCompteur")) { container.style.cursor = 'cell' } else { container.style.cursor = 's-resize' };
    }
...
function vueChange(posX, posY, posZ, upX, upY, upZ) {
    ...
    if (Math.abs(camera.position.x * camera.position.y * camera.position.z) > 1200) { pasChange = 1; }
...
var parcoursCurseurRetenueTrans = [],
    retTransAct = [],
    retAct = [],
    parcoursDoubleEngr = [],
    parcoursCurseurRetenue = [],
    parcoursAccu = [],
    parcoursAxeEngr = [],
    parcoursEngrCompt = [],
    parcoursMavinelle = [],
...
var parcoursBarreAddSub = [],
    parcoursAxePlateau = [],
    parcoursPeigneAccu = [],
    parcoursPeigneCompteur = [],
    parcoursPignonAccu = [],
    parcoursPignonCompteur = [];
...
function translaterPiece(vitesse, objet, axe, signe, i, tab) {
    ...
    if (signe == '+' && tab[i] > 0) {
        if (axe == 'y') { objet.position.y += a; } else if (axe == 'x') { objet.position.x += a; } else if (axe == 'z') { objet.position.z += a; }
        tab[i] -= a;
...
function tournePiece(nb20emeTours, objet, axe, signe, vitesse, i, tab) {
    ...
    var a = tps * velocity * vitesse;
    if (signe == '+' && tab[i] > 0) {
        if (axe == 'y') { objet.rotation.y += a; } else if (axe == 'x') { objet.rotation.x += a; } else if (axe == 'z') { objet.rotation.z += a; }
...
function velocite() {
    if (document.getElementById('lente').checked) { velocity = 0.0025; } else { velocity = 0.05; }
}
var parcoursPlaque = [];
